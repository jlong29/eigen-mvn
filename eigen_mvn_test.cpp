...
/*
Compile:
g++ -std=c++11 -O3 -o eigen_mvn_test eigen_mvn_test.cpp eigen_mvn.cpp -I$EIGEN_ROOT_DIR `pkg-config --cflags-only-I --libs gtest`
...
TEST(TestMvn, TestPdf)
{
	...
	test << 0.0f, 0.0f;
	EXPECT_NEAR(mvn.pdf(test), 0.16f, 1e-4f);

	test << -0.6f, -0.6f;
	EXPECT_NEAR(mvn.pdf(test), 0.1153f, 1e-4f);
}
...
TEST(TestMvn, TestSampling)
{
	...
	EXPECT_TRUE((approx_mean-mu).cwiseAbs().maxCoeff() < convergChk0);
	EXPECT_TRUE((approx_sigma-sig).cwiseAbs().maxCoeff() < convergChk1);
}
